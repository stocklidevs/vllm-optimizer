...
Release label: `0.56.8 public alpha`
...
- `.github/ISSUE_TEMPLATE/model_validation_report.md`
...
- `vllm-optimizer --version` prints `0.56.8`.
- `pytest` passes.
...
- The best live evidence comes from one GX10-style setup and should be treated
  as hardware/workload specific. The optimizer is intended to run against other
  SSH-accessible Linux NVIDIA/vLLM hosts after adapting local config, serve
  profiles, and conservative sweep ranges.
...
4. Tag `v0.56.8-alpha` after the final release-check passes.
